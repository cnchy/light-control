		...
		<view class="modelBox" v-if="showHide">
			...
			<view class="model">
				...
				<view class="modelInput">
					<input  placeholder-class="inputStyle" v-model="modelValue"  type="number"/>
				</view>
...
<script>
	export default {
			...
			inputValue(newVal, oldVal) {
				if (+newVal !== +oldVal) {
					...
					if(oldVal != -999){
						...
						// 	title:"触发了"
						// })
						this.$emit('change', {detail:{value:a}})
					}
				}
			}
		},
		...
		methods: {
			...
			// 确定
			confirm(){
				if(this.modelValue -this.max> 0){
					this.inputValue=Math.round(this.max)
				}else{
					this.inputValue=Math.round(this.modelValue)
				}
				this.showHide=false
			}
		}
...
<style>
	...
	.uni-numbox__minus:active,
	.uni-numbox__plus:active {
